...
- **git diff must be non-empty:** the harness runs \`git diff HEAD\` after your session. If it is empty your edits were no-ops, were reverted, or targeted the wrong path. The harness will detect this, reset your edit state, and force another pass — so verify your changes actually landed before stopping.
